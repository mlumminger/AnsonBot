function economyCommands() {
  ...
  if (dividedMessage[0] == `${pre}weekly`) {
    var index = GetIndexFromUserID(msg.author.id, true, msg);
    var user = database.users[index];
  ...
  if (dividedMessage[0] == `${pre}withdraw`) {
    var amount = dividedMessage[1]
    var index = GetIndexFromUserID(msg.author.id, true, msg);
    var user = database.users[index];
  ...
  if (dividedMessage[0] == `${pre}deposit`) {
    var amount = dividedMessage[1]
    var index = GetIndexFromUserID(msg.author.id, true, msg);
    var user = database.users[index];
  ...
  if([`${pre}leaderboard`, `${pre}lb`].includes(dividedMessage[0])) {
